...
## Checkout Order Summary Editing + PDP "Go to Cart" (2026-09-18)

- Checkout order summary lines now have a qty stepper (−/+ capped by `cartQtyCap`) and a delete button. Guests call `zCart.updateQuantity/removeItem`; logged-in users optimistically mutate the local `cart` state then POST the full items to `/api/cart` (`persistCartItems`), taking the server response (re-priced + stock-clamped) as authoritative, re-fetching on failure.
- **Gotcha**: the Zustand cart store matches lines with STRICT equality on `product.id` (`item.product.id === productId`), but the checkout derives the guest cart with `product.id = String(i.product.id)`. Passing the derived id breaks matching (number 5 vs string "5"). Resolve the real store line by composite key (`cartMergeKey()`) before calling the store mutators — see `guestStoreItem()` in `checkout/page.tsx`.
- **Guest product shape**: the derived guest cart must spread `...i.product` so `trackQuantity`/`quantity`/`colorVariants` survive — `stockForColor()` and `cartQtyCap()` silently no-op without them (guests otherwise never see low-stock hints and cap at 10).
- **Race safety**: `persistCartItems` uses a `cartSyncSeq` ref so only the LATEST request may write state back (`setCartSaving(false)` and `setCart` are gated on `seq === cartSyncSeq.current`); otherwise rapid ± clicks settle on the last-resolved response, not the last edit.
- **Clamp/error UX**: after the server responds, diff server vs optimistic quantities and surface a `cartNotice` (auto-dismiss ~4s) for clamped lines; if both POST and refetch fail, roll back to `cartSnapshotRef` (pre-edit) instead of silently diverging.
- **Coupon**: persist `appliedCoupon?.id` (not `cart.coupon.id`) as `couponId` during edits, and re-validate the applied coupon against the new subtotal (min-cart-value can be violated by removals) — auto-remove + notice if invalid. Coupon chip renders the LIVE `discount`, not the apply-time `appliedCoupon.discount`.
- PDP (`ProductActions.tsx`): when the selected product + color is already in the bag, the CTA renders "Go to Cart" and calls `openCart()` (via `useUI`) instead of re-adding. `isInCart` is variant-aware (recomputes on color switch); the transient `addedState`/"Added!" flash was removed in favor of the persistent `isInCart` state. `handleBuyNow` also skips re-adding when `isInCart` (no silent qty bump) and the OOS branch shows a "Go to Cart" button when the item is in the bag.
- Empty cart after removal redirects to `/` (both guest and logged-in), matching the existing empty-cart-at-load behavior.

## Product Code and GST Number (2026-09-18)

- **Product codes** are unique identifiers for inventory tracking (format `SHG-XXXXX`). Required on all products, unique at DB level, indexed for fast lookups.
- **GST number** is business tax identification stored in Site Settings, displayed on invoices/receipts. Optional field with format validation (15 chars: 2-digit state + 10-digit PAN + entity code + Z + checksum, e.g., `22AAAAA0000A1Z5`).
- Migration `20260918_000000_add_product_code_and_gst`: adds `products.product_code` (varchar, unique, NOT NULL, indexed), auto-generates codes for existing products based on `created_at` order, adds `orders_items.product_code` (snapshot at purchase, varchar, indexed), adds `site_settings.gst_number` (varchar, optional), includes version history tables (`_products_v.version_product_code`, `_site_settings_v.version_gst_number`). Down migration is idempotent.
- **Snapshot pattern**: Order items capture `productCode` at purchase time (via `resolveCurrentPrices` in Razorpay verify endpoint), protecting against product deletions/renames. The price resolution now returns `{ price, productCode }` instead of bare price.
- **Receipt integration**: `GET /api/orders/receipt?orderNumber=...&email=...` fetches business info from Site Settings (siteName, contactEmail, contactPhone, address, gstNumber, logo), passes it to PDF generator. Receipts display product codes in the items table and business info (including GST number) in the footer. Filename is `{siteName}-{orderNumber}-receipt.pdf`.
- **Email templates**: Order confirmation emails show product codes under item names.
- **Type safety improvements**: Receipt route defines `SiteSettingsResponse` interface instead of `as any` casts. Business info defaults centralized in `receipt.ts` (`DEFAULT_SITE_NAME`, `DEFAULT_SUPPORT_EMAIL`, `DEFAULT_SITE_URL`) and imported by route.
- **Order details page**: Displays product code under item name if available (`account/orders/[id]/page.tsx`).
- **Test updates**: `cart-prices.test.ts` expects `ResolvedProductInfo` objects (`{ price, productCode }`), `Products.test.ts` verifies `productCode` field is unique/indexed/required (33 fields total), `SiteSettings.test.ts` updated for `gstNumber` field (25 fields total).
- **Database indexes**: Both `products.product_code` and `orders_items.product_code` are indexed for search performance (admins may query orders by product code).
