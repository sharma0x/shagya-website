...
export function ProductFilters({
  ...
  const [excludeOOS, setExcludeOOS] = useState(
    searchParams.get('excludeOOS') === 'true',
  )
  ...
  */
  ...
  const buildQuery = useCallback(() => {
    ...
    else if (contextFilter?.fabric && !initialRender.current)
      params.set('fabric', '')
    if (weave.length) params.set('weave', weave.join(','))
    else if (contextFilter?.weave && !initialRender.current)
      params.set('weave', '')
          ...
          {FABRIC_OPTIONS.filter(
            ...
            >
              <input
                ...
                onChange={() => toggleArrayFilter(opt.value, fabric, setFabric)}
                className={checkboxClass}
              />
          ...
          return (
            <>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected
                            ? 'bg-brand-50 ring-brand-300 ring-1'
                            : 'hover:bg-neutral-50',
                        )}
                      >
                        <span
              ...
              {showCount > INITIAL_COLOR_COUNT && (
                ...
                >
                  {showAllColors ? 'Show Less' : `Show All ${showCount} Colors`}
                </button>
              )}
            </>
          )
      ...
      >
        <div ref={cityDropdownRef} className="relative">
          <button
            ...
            className={cn(
              'font-body flex h-9 w-full items-center justify-between rounded-lg border bg-white px-3 text-xs transition-all outline-none',
              cityDropdownOpen
          ...
          {cityDropdownOpen && (
            <div className="absolute top-full right-0 left-0 z-30 mt-1 overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-lg">
              <div className="max-h-48 overflow-y-auto">
                ...
                {(facets?.cities || []).map((c) => (
                  ...
                  >
                    ...
                    {c.count > 0 && (
                      <span className="ml-2 shrink-0 text-[10px] text-neutral-400 tabular-nums">
                        {c.count}
                      </span>
                    )}
                  </button>
            ...
            <div className="flex items-center justify-between border-b border-neutral-100 pb-4">
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="font-display text-brand-600 hover:text-brand-700 cursor-pointer text-[10px] font-semibold tracking-wider uppercase transition-colors"
                >
                  Clear All
                </button>
              )}
