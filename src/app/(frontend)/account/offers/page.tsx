...
function CouponGridCard({ coupon }: { coupon: any }) {
  ...
  return (
    <div className="flex flex-col rounded-xl border border-neutral-100 bg-white p-4 shadow-xs">
      <span className="font-display text-brand-700 bg-brand-50 inline-flex h-7 w-fit items-center rounded-lg px-2 text-[10px] font-bold tracking-wide">
        {formatDiscount()}
      </span>

      <p className="mt-3 font-mono text-sm tracking-wider text-neutral-800">
        {coupon.code}
      </p>
      ...
      <div className="font-body mt-2 space-x-2 text-[10px] text-neutral-400">
      ...
      <button
        onClick={handleCopy}
        className={cn(
          'font-display mt-auto inline-flex items-center gap-1 pt-3 text-[10px] font-semibold tracking-wider uppercase transition-colors',
        ...
        {coupons.length > 0 ? (
          ...
          <div className="rounded-2xl border border-neutral-100 bg-white p-12 text-center shadow-xs">
            ...
            <p className="font-body mt-2 text-sm text-neutral-500">
              Active coupons and special discounts will appear here. Keep an eye
              out for seasonal sales!
            </p>
            <Link
              href="/category/all"
